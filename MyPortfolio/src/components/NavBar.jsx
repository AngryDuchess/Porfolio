...
export default function NavBar() {
  ...
  const isActiveTab = (path) => {
    if (path.startsWith('#')) {
      return window.location.hash === path ? 'tab-item-shadow font-medium text-portfolioWhite bg-gray-400 rounded-full' : '';
    }
    else
      return location.pathname === path ? ' font-semibold text-portfolioWhite' : '';
  };
  ...
  return (
    <>
      <header className="w-screen backdrop-blur-sm xl:sticky z-20 bottom-0 fixed xl:top-0 px-4 bg-portfolioWhite backdrop-filter bg-opacity-50 pb-8 xl:pb-0 xl:pt-8">
        <nav className="bg-gray-900 backdrop-filter backdrop-blur-sm nav-shadow bg-opacity-60 border-b border-gray-600 mx-0 md:mx-auto px-2 py-2 max-w-3xl rounded-full">
          <div className="flex items-center justify-between">
            <a href="/">
              <img src="/favicon.svg" alt="favicon" />
            </a>
            <ul className="text-gray-300 flex gap-4">
              <li className={`${isActiveTab('#hero')} py-1 px-3 hover:bg-gray-400 hover:rounded-full`}>
              <a href={window.location.pathname === '/' ? '#hero' : '/'} className="">Home</a>
              </li>
              <li className={`${isActiveTab('#work')} py-1 px-3 hover:bg-gray-400 hover:rounded-full`}>
                <a className="" href="#work">Work</a>
              </li>
              <li className={`${isActiveTab('#about')} py-1 px-3 hover:bg-gray-400 hover:rounded-full`}>
              ...
              {isPopupOpen && (
                <div ref={popupRef} className="absolute z-30 popup xl:top-14 right-0 bg-portfolioWhite shadow-lg w-full max-w-36 py-3 rounded-md">
                  <ul className="text-sm text-left">
                    <li className="p-2 hover:bg-blue-100 hover:mx-1 hover:rounded-md cursor-pointer flex gap-2" onClick={handleModal}>
                      <CallCalling size="16" color="#2a2a2a" /> Book a call
                    </li>
                    <li className="p-2 hover:bg-blue-100 hover:mx-1 hover:rounded-md">
                      <a className="flex gap-2" href="mailto:mahamahamida@gmail.com">
                        <Messages2 size="16" color="#2a2a2a" /> Send email
                      </a>
                    </li>
                    <hr className="mt-4" />
